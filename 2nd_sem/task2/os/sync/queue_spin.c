...
queue_t* queue_init(int max_count) {
    ...
    if (pthread_spin_init(&w->lk, PTHREAD_PROCESS_PRIVATE) != 0) {
        perror("pthread_spin_init"); abort();
    }
...
void queue_destroy(queue_t *q) {
    ...
    pthread_spin_lock(&w->lk);
    qnode_t *n = q->first;
    while (n) { qnode_t *next = n->next; free(n); n = next; }
    pthread_spin_unlock(&w->lk);

    pthread_spin_destroy(&w->lk);
    free(w);
}
...
int queue_add(queue_t *q, int val) {
    ...
    if (!node) { pthread_spin_unlock(&w->lk); perror("malloc node"); abort(); }
